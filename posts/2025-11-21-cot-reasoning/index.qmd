...
description: "Paper notes on CoT-decoding and how premise order affects LLM reasoning"
author: "Siva"
date: 2025-11-21
categories: [notes, llm, paper-notes]
---
...
$$
\Delta_{k, \text{answer}} = \frac{1}{|\text{answer}|} \sum_{x_t \in \text{answer}} \bigl(p(x_t^1 \mid x_{<t}) - p(x_t^2 \mid x_{<t})\bigr)
$$
...
![CoT decoding example](cot_example.png){fig-alt="Illustration of CoT vs greedy decoding paths"}
...
![CoT vs greedy comparison](cot_vs_greedy.png){fig-alt="Chart comparing CoT-decoding to greedy decoding"}

### The Fragility of LLM Reasoning

**Paper 2:**
[Premise Order Matters in Reasoning with Large Language Models](https://arxiv.org/pdf/2402.08939)

The core point of this paper is LLMs' frailty and their dependence on the order in which premises are presented to them.
...
![U-curve for GPT-4 premise order](ucurve.png){fig-alt="U-shaped performance curve for GPT-4 across premise orders"}
...
![Fact hallucination example](hallucination.png){fig-alt="Diagram of fact hallucination under disordered premises"}

Finally, these behaviors shouldn't be confused with the **"Lost-in-the-middle"** effect, where models forget information in the middle of long contexts. However, the benchmark problems here are short (<300 tokens). Experiments placing rules at the beginning, middle, or end showed little difference compared to the massive impact of scrambling the order.
